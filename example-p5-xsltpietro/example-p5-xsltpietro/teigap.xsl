...
<!-- $Id: teigap.xsl 2271 2014-10-18 19:24:03Z pietroliuzzo $ -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="http://fxsl.sf.net/"
   ...
   <xsl:function name="EDF:dotchar" as="xs:string">
      ...
      <xsl:variable name="dot">
         <xsl:choose>
            ...
            <xsl:when test="$leidenStyle = ('panciera', 'ila') and $gapReason='illegible'"><xsl:text>+</xsl:text></xsl:when>
      ...
      <xsl:choose>
            ...
            <xsl:choose>
               ...
               <xsl:when test="number(@quantity) &gt; $cur-max or (number(@quantity) &gt; 1 and @precision='low')">
                  <xsl:choose>
                     ...
                     <xsl:when test="$parm-leiden-style = 'panciera' and @reason='illegible'">
                        <xsl:sequence select="f:repeat($cur-dot, @quantity)"/>
                     </xsl:when>
                     <xsl:when test="$parm-leiden-style = 'panciera' and not(@reason='illegible')">
                        <xsl:text>c. </xsl:text>
                        <xsl:value-of select="@quantity"/>
                     </xsl:when>
                     <xsl:when test="$parm-leiden-style = 'london'">
         ...
         <xsl:when test="@atLeast and @atMost and not($parm-leiden-style=('edh-names','edh-itx'))">
            ...
            <xsl:choose>
               ...
               <xsl:when test="$parm-leiden-style = 'ila'">
                  <xsl:text>[</xsl:text>
                  <xsl:value-of select="@atLeast"/>
                  <xsl:text>-</xsl:text>
                  <xsl:value-of select="@atMost"/>
