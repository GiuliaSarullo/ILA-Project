...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="t:choice">
       ...
       <xsl:choose>
          <xsl:when test="child::t:sic and child::t:corr and (child::t:supplied[@evidence='previouseditor'] or child::t:abbr)">
             <xsl:value-of select="translate(t:sic, $all-grc, $grc-upper-strip)"/><xsl:text>(</xsl:text><xsl:value-of select="translate(t:corr, $all-grc, $grc-upper-strip)"/><xsl:text>)</xsl:text>
          </xsl:when>
